...
public class JDBCDataManager implements DataManager {
		...
		try {
			...
			statement = connection.prepareStatement(
				"INSERT INTO employees ( " +
					"first_name, second_name, hire_date, salary, " +
					"jobtitles_id, departments_id ) VALUES ( " +
					"?, ?, ?, ?, ?, ?, ? );"
			);
			statement.setString(1, employee.firstName);
			statement.setString(2, employee.secondName);
			statement.setDate(3, new java.sql.Date(employee.birthdate.getTime()));
			statement.setDate(4, new java.sql.Date(employee.hiredate.getTime()));
			statement.setInt(5, employee.salary);
			statement.setInt(6, getJobTitleId(employee.jobTitle));
			statement.setInt(7, departmentsId);
  ...
  public void add(Department department) throws RemoteException {
		...
		try {
			connection = dataSource.getConnection();
			statement = connection.prepareStatement(
				"INSERT INTO departments (name, Description) VALUES ( ?, 'x' );"
			);
			statement.setString(1, department.name);
			statement.executeUpdate();
		} catch (Exception e) {
